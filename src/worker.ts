...
/**
 * Represents the Worker environment bindings.
 * These fields might be undefined if not set in wrangler.jsonc or environment variables.
 */
interface Env {
...
/**
 * Represents an inbound email message from Cloudflare Email Workers.
 */
interface ForwardableEmailMessage {
	/**
	 * A map-like object containing the email headers. Each key is a header name, and the value is the header value.
	 */
	headers: Map<string, string>;

	/**
	 * A ReadableStream representing the raw email content.
	 */
	raw: ReadableStream<Uint8Array>;

	/**
	 * The total size of the raw email stream, in bytes.
	 */
	rawSize: number;

	/**
	 * Marks the email message as rejected, providing a reason for the rejection.
	 * @param reason A string describing why the email is being rejected.
	 */
	setReject: (reason: string) => void;
}

/**
 * Represents the Cloudflare Execution Context for handling email events.
 */
interface ExecutionContext {
	/**
	 * Signals that work can continue indefinitely, even after returning a response.
	 * Typically used to handle asynchronous tasks without delaying the user-facing response.
	 * @param promise A Promise to track.
	 */
	waitUntil: (promise: Promise<unknown>) => void;
}

/**
 * Represents a processed version of the email, containing parsed headers and body.
 */
...
/**
 * Represents the context shared across processors.
 */
...
/**
 * Represents the result of a processing step.
 */
 ...
 * @param streamSize The expected size of the stream in bytes.
 * @return A Promise that resolves with the resulting bytes from the stream.
 */
 ...
 * @return The base64-encoded representation of the original data.
 */
...
/**
 * Defines a base class for all email processors.
 * Subclasses must implement the `process()` method to perform specialised logic.
 */
class BaseEmailProcessor {
	/**
	 * The processing context shared among processors.
	 * @protected
	 */
	protected context: ProcessingContext;

	/**
	 * Creates a new instance of BaseEmailProcessor.
	 * @param context The shared processing context, containing email data, environment, etc.
	 */
	...
	/**
	 * Processes the email. Subclasses must override this method.
	 * @return A Promise resolving to an object indicating if processing should continue, and any rejection reason.
	 */
...
class SpamCheckProcessor extends BaseEmailProcessor {
	/**
	 * Executes spam checks against the email.
	 * @return A Promise resolving to a ProcessorResult indicating whether or not to continue.
	 */
	async process(): Promise<ProcessorResult> {
...
class ParsingProcessor extends BaseEmailProcessor {
	/**
	 * Reads and parses raw email data from the message.
	 * @return A Promise that resolves to a ProcessorResult indicating if processing should continue.
	 */
	async process(): Promise<ProcessorResult> {
		const { message } = this.context;
...
class DomainCollapseProcessor extends BaseEmailProcessor {
	/**
	 * Applies domain collapsing to the parsed email headers if configured in the environment.
	 * @return A Promise that resolves with a ProcessorResult object.
	 */
	async process(): Promise<ProcessorResult> {
...
class SubaddressingProcessor extends BaseEmailProcessor {
	/**
	 * Detects subaddress tags in the To header and updates the Subject with the subaddress tag if found.
	 * @return A Promise that resolves with a ProcessorResult object.
	 */
	async process(): Promise<ProcessorResult> {
...
class PreDeliveryProcessor extends BaseEmailProcessor {
	/**
	 * Re-serialises the parsed email back to a raw format for final delivery.
	 * @return A Promise that resolves with a ProcessorResult object.
	 */
	async process(): Promise<ProcessorResult> {
		const { parsedEmail } = this.context;
...
class DeliveryProcessor extends BaseEmailProcessor {
	/**
	 * Delivers the raw email to an Odoo CRM instance via XML-RPC.
	 * @return A Promise that resolves with a ProcessorResult indicating success or failure.
	 */
	async process(): Promise<ProcessorResult> {
		...
		try {
			...
			console.error('DeliveryProcessor:', err);
			return { canContinue: false, rejectReason: `Unable to deliver to CRM. ${err.message}` };
		}
...
/**
 * Represents a set of CRM server connection options.
 */
interface CrmServerOptions {
	database: string;
	userid: string;
	password: string;
	host: string;
...
class CrmServerHandler {
	 ...
	 *
	 * @param rawEmail The raw email data as a Uint8Array.
	 * @return A Promise that resolves when the email has been processed or rejects on error.
	 */
  ...
  <params>
    ...
    <param><value><struct></struct></value></param>
		...
		try {
			...
			throw new Error(`Unable to communicate with CRM server: ${err.message}`);
		}
	}
	 ...
	 * @param data The response text body.
	 * @private
	 */
	async #validateResponse(response: Response, data: string): Promise<void> {
		...
		if (data.includes('<fault>')) {
			...
			const faultStringRegex =
				/<name>\s*faultString\s*<\/name>\s*<value>\s*<string>\s*([\s\S]*?)\s*<\/string>/i;
...
/**
 * Parses the headers and body from raw email data.
 ...
 *
 * @param rawEmail The raw email data as a Uint8Array.
 * @return An object with parsed headers and body.
 */
 ...
 * @return An object containing displayName and address.
 */
...
/**
 * Re-serialises an address into "Name <address>" format if a display name is provided,
 * otherwise returns just the address.
 *
 * @param displayName The address's display name.
 * @param address The address's email portion.
 * @return A string suitable for use in an email header.
 */
 ...
 *
 * @param str A string that may or may not be enclosed in quotes.
...
/**
 * Parses a domain collapse map from a string that may be comma or semicolon separated.
 * For example, "suppliers.example.invalid=suppliers@example.invalid,team.example.invalid=allstaff@example.invalid".
 *
 * @param str The domain collapse mapping string.
 * @return An object mapping lowercased domain -> target address.
 */
 ...
 *
 * @param headers An array of objects representing email headers.
 * @return An object containing the detected subaddressTag and the updated headers.
 */
 ...
 *
 * @param headers The array of email headers.
 * @param tag The subaddress tag to prepend, if any.
 * @return The updated array of headers with the possibly-modified Subject.
 */
 ...
 *
 * @param headers The email headers to re-serialise.
 * @param body The email body as a Uint8Array.
 * @return A Uint8Array containing the re-serialised email with CRLF line endings.
 */
...
/**
 * Default export containing the Cloudflare Worker-style email handler.
 */
export default {
	/**
	 * Cloudflare Worker-style email handler.
	 *
	 * @param message The email message to process, with headers, raw data, etc.
	 * @param env The Worker environment, containing any relevant bindings or settings.
	 * @param ctx The execution context for asynchronous tasks.
	...
	async email(
		...
		for (const processor of processors) {
			try {
				...
				console.error(`Error in ${processor.constructor.name}`, e);
			}
		}
